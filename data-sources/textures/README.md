...
> **Current phase-7 delivery:** [PHASE7-DELIVERY.md](PHASE7-DELIVERY.md) links the editing ZIP, 54 era guides and verification results. Five partial regional ice suites (15 PNGs) are available; 49 eras, all coastlines and seasonal sea ice remain unavailable. Read [EDITOR-GUIDE.md](EDITOR-GUIDE.md) for layer usage and [REPRODUCING-MASKS.md](REPRODUCING-MASKS.md) for the `package-delivery`/`validate-delivery` commands. The [phase-5 review](PHASE5-VALIDATION.md) rejected the coastline candidates and left ice scientific acceptance pending; older progress notes below do not supersede those findings.
