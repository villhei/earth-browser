...
Version 2: four independent **4096 × 2048 PNG** sets, aligned to the existing
...
- **`-depth-change-8bit.png`**: normalized white overlay. Transparent is zero;
  opaque white is the magnitude of that era's assumed sea-level shift (7, 55, 25 or
...
- **`-excluded.png`**: opaque white where modern bright snow or the available era ice
  mask suppressed output; transparent elsewhere. Transparency does not establish scientific validity.
...
Zero values also include unsupported/excluded areas; they are not evidence of no change.
