...
describe("terrain overlay", () => {
  it.each([10000, 8000, 5000])("automatically selects the matching mask for %i BCE Blue Marble", (year) => {
  ...
  it.each([10000, 8000, 5000])("removes the %i BCE mask when disabled and restores it when enabled again", (year) => {
  ...
  it.each([undefined, "world-bc123000", "world-bc4000", "world-bc3000", "world-2010", "toString"])(
  ...
  it.each([GlobeTexture.EARTH_DAY, GlobeTexture.EARTH_NIGHT, GlobeTexture.EARTH_DARK])(
    "removes the terrain overlay when switching to %s", (texture) => {
      for (const year of [10000, 8000, 5000]) {
